    ...
    try {
        const httpRequest = axios.create({
            baseURL:
                userService === "Ecommerce"
                    ? process.env.ECOMMERCE_BASE_URL
                    : "", //cần truyền serviceName để call api của service đó
        });
...
const getListClaim = (user) => {
    return {
        ...
        role: user.role,
        serviceName: user?.Service?.serviceName || user.serviceName,
        serviceUrl: user?.Service?.serviceUrl || user.serviceUrl,
    };
};
...
const forgotPassword = async (data) => {
    try {
        ...
        await sendMailAsync(
            user.email,
            "Reset your password",
            `Click here to reset your password. <a href="${message}">Click here!</a>`
        );
...
const enableF2A = async (params) => {
    ...
    const data = JSON.stringify({
        ...
        to: [
            {
                type: "external",
                number: params?.phone.startsWith("0")
                    ? params?.phone.replace("0", "84")
                    : params?.phone,
                alias: "TO_NUMBER",
            },
        ],
...
const verifyOtp = async (data, verifyType) => {
    ...
    try {
        ...
        if (
            otpValidateResult.statusCode === 404 ||
            otpValidateResult.statusCode === 401
        ) {
            await t.commit();
        ...
        if (verifyType === "verify-f2a") {
            ...
            return {
                statusCode: 200,
                message: data?.isF2A
                    ? "Enabled F2A successfully."
                    : "Disabled F2A successfully.",
                user: userInfoExtend,
            };
...
const getGoogleProfile = async (code) => {
    ...
    } = await axios.post(`${process.env.GOOGLE_TOKEN_URI}/token`, null, {
        params: {
...
const loginGoogle = async (params) => {
    ...
    try {
        ...
        if (userLink && !userLogins) {
            ...
            const newUser = await postUserInfo(
                newUserWithProvider,
                params.serviceName
            );
            const newProvider = {
        ...
        if (userLogins) {
            ...
            if (data.type == "login") {
                ...
                const newProvider = {
                    ...
                    accountName: userProfile.name,
                    isUnlink:
                        !!userLink?.password || !!userLink.UserLogins?.length,
                };
                await db.UserLogin.create(newProvider, { transaction: t });
...
const refreshToken = async (refreshToken, remember) => {
    const t = await sequelize.transaction();
    try {
        if (!refreshToken) {
            await t.commit();
